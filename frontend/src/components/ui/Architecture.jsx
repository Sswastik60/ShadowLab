    ...
    <div className="glass-panel rounded-2xl p-6 relative overflow-hidden">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          {isCacheActive && (
            <span className="flex items-center gap-1 text-emerald-400 bg-emerald-950/60 px-2 py-0.5 rounded-md border border-emerald-500/30">
              <span className="h-2.5 w-2.5 rounded-full bg-emerald-400 animate-pulse"></span> Valkey Cache Active
            </span>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col items-center justify-center py-6 gap-6 min-h-[300px] relative">
        ...
        <div className="flex flex-col items-center">
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-neutral-900 border border-neutral-800 text-xs font-semibold text-neutral-300 shadow-md">
            <Globe className="h-4 w-4 text-emerald-400" />
            <span>User Ingress Traffic (1,050 QPS)</span>
          </div>
          <div className="h-8 w-0.5 bg-gradient-to-b from-emerald-500 to-emerald-400 my-1 animate-pulse"></div>
        </div>
        ...
        <div className="w-full max-w-md flex justify-around relative my-1">
          ...
          {isCacheActive && <div className="h-10 w-0.5 bg-emerald-500 animate-pulse"></div>}
        </div>

        {/* Bottom Nodes: PostgreSQL & Valkey */}
        <div className="w-full max-w-lg grid grid-cols-2 gap-4">
          {/* Database */}
          <div className="flex items-center gap-3 p-4 rounded-xl bg-neutral-950 border border-neutral-900 text-neutral-200">
            <div className="p-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400">
              <Database className="h-5 w-5" />
            </div>
            <div>
              <p className="font-bold text-xs text-white">PostgreSQL 16</p>
              <p className="text-[10px] text-neutral-400">Primary Database Storage</p>
          ...
          <div className={`flex items-center gap-3 p-4 rounded-xl border transition-all ${
            isCacheActive 
              ? 'bg-emerald-950/40 border-emerald-500/40 text-emerald-200 glow-emerald' 
              : 'bg-neutral-950 border-neutral-900 text-neutral-600 opacity-60'
          }`}>
            <div className={`p-2 rounded-lg ${isCacheActive ? 'bg-emerald-500/20 text-emerald-400' : 'bg-neutral-900 text-neutral-500'}`}>
              <Zap className="h-5 w-5" />
            </div>
            <div>
              <p className="font-bold text-xs text-white">Valkey Cache</p>
              <p className="text-[10px] text-neutral-400">In-Memory Cache Tier</p>
              {isCacheActive ? (
                <span className="inline-block mt-1 text-[10px] font-mono text-emerald-400">⚡ 87% Hit Rate</span>
              ) : (
                <span className="inline-block mt-1 text-[10px] font-mono text-neutral-500">⚪ Not Added</span>
              )}
            </div>
          </div>
        </div>
